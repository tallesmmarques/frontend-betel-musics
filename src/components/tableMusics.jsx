...
import { formatDistanceToNow, isDate, parse } from "date-fns"
...
function TableMusics({ musics, title, ministerio }) {
  ...
  const [tableh, setTableh] = useState(["Música", "Artista", "Gênero", "Alber", "Lucymeire", "Adolescentes", "CifraClub", "Youtube", ""])
  ...
  useEffect(() => {
    switch (filter) {
      case "sdn-alber":
        ...
        break;
      case "sdn-lucimeire":
        setTableh(["Música", "Artista", "Gênero", "Lucymeire", "Tocada há", "CifraClub", "Youtube", ""])
        break;
      case "adolescentes":
      ...
      default:
        setTableh(["Música", "Artista", "Gênero", "Alber", "Lucymeire", "Adolescentes", "CifraClub", "Youtube", ""])
        break;
    }
  }, [filter])
              ...
              <Tr key={music.id}>
                ...
                {filter !== "all" && (
                  <Th textTransform="none" minW="50px" fontSize="14px" fontWeight="light"
                    bg={lastPlayed ? (
                      differenceInWeeks(new Date(), lastPlayed) < 1 ? "red.100" : (
                        differenceInWeeks(new Date(), lastPlayed) < 2 ? "orange.100" : "green.100"
                      )) : "blue.100"}
                  >{
                      lastPlayed ? formatDistanceToNow(lastPlayed) : "Não tocada"
                    }</Th>
                )}
